...
public class UpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    Movie m = new Movie(title,ids,language,durat,kind,new Adresse(numI,street,postalCodeI,city,country),minAgeI,description,new Programes(parsed1,parsed2,dayListS,startingDate,endingDate),author,act1,city);
	    String urlUpd =  baseURI+ids;
	    WebTarget webTarget = client.target(urlUpd);
	    Response resp = webTarget.request().put(Entity.entity(m, MediaType.APPLICATION_JSON),Response.class);
